...
只要是处理字符串的函数（printf, strcpy, strlen），约定好：拿到首地址后，就无脑往后读，直到遇到数字 0 (\0) 为止。printf("%s"),天生接收指针。  
2.一个字节=8位比特（uint8），半个字=2个字节（uint16），一个字=4个字节（uint32）。接收数组大小也得对应上。  
3.表驱动法：  
...
6.extern，最好写在.h文件中。要用的.c文件里也行，但是不规范  
7.数组作为函数参数传递时，它会退化为指向数组首元素的指针  
## 1. 任务流程
