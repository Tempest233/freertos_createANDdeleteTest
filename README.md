...
1. 配置DMA参数(时钟，各属性，连接DMA与外设(USART)，优先级，开启中断)，串口同理。最后再开一次接收中断开关
2. 先写接收中断的逻辑：收到DMA发来的数据，传入队列，再次开启接收中断
3. 编写发送函数：发现队列存在数据，判断发送锁是否锁上，把队列数据拷到缓冲区，把缓冲区数据送入DMA异步发送
4. 编写发送中断的逻辑：发送完成，把锁打开
5. 启动调度器  
